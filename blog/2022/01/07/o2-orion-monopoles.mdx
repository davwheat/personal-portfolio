...
"Beacon" was the codename for a series of joint upgrade plans made between O2 and Vodaone to provide LTE to the whole of the UK.
...
This indicates a possibly deterirating relationship between the two operators.
...
I am yet to fieldtest my site, so I cannot confirm this is the exact deployment. **n78 5G is not yet active on the site** as the remote radio
unit is missing from the monopole, as you can see below, and the 3500 MHz feeders remain disconnected next to it. This RRU is set to be installed
on 30 Jan 2022, according to information from [one.network](https://one.network/).
...
Thanks go to Peter Clarke, Jake (Undersc0re), Derico and many others for help in piecing together sections of this article.
